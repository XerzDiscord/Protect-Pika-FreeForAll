...
module.exports = function (client) {
    ...
    const jtch = db.get(`jointocreatech_${message.guild.id}`)
    if(!jtch || db.get(`jointocreate_${message.guild.id}`) !== "on") return;
  ...
  new Promise(resolve => {
    setInterval(() => {
      resolve(2);
        try{
          const guild = client.guilds.cache.get("811979327250890753");
 ...
 client.on("voiceStateUpdate", (oldState, newState) => {
  ...
  if (!oldState.channelID && newState.channelID) {
    if(newState.channelID !== jtch) return;  //if its not the jointocreatechannel skip
